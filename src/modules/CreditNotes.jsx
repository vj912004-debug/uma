...
const CreditNotes = () => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (isEditing) {
      updateItem('creditNotes', isEditing, { ...formData, id: isEditing });
    } else {
      updateData('creditNotes', { ...formData, id: Date.now().toString() });
      incrementSerial('CN');
    }
    ...
    <div>
      <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
        <div>
          ...
          <p style={{ color: 'var(--text-muted)' }}>Manage financial credit notes for customers/suppliers.</p>
        </div>
                  ...
                  <tr key={note.id}>
                    ...
                    <td>
                      <div style={{ display: 'flex', gap: '0.5rem' }}>
                        ...
                        <button onClick={() => deleteDataSoftly('creditNotes', note.id)} style={{ background: 'transparent', border: 'none', color: 'rgba(239, 68, 68, 0.6)', cursor: 'pointer' }}><Trash2 size={16} /></button>
                      </div>
                    </td>
                  </tr>
                ))
      ...
      {isModalOpen && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.85)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100, backdropFilter: 'blur(5px)', padding: '2rem' }}>
          <div className="premium-card" style={{ width: '600px', maxWidth: '95%' }}>
            <h2 style={{ marginBottom: '1.5rem' }}>{isEditing ? 'Edit Credit Note' : 'Add Credit Note'}</h2>
