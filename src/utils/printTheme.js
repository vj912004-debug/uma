...
export const IC = {
  ...
  users: ic('M16 11c1.7 0 3-1.3 3-3s-1.3-3-3-3-3 1.3-3 3 1.3 3 3 3zM8 11c1.7 0 3-1.3 3-3S9.7 5 8 5 5 6.3 5 8s1.3 3 3 3zm0 2c-2.3 0-7 1.2-7 3.5V19h14v-2.5C15 14.2 10.3 13 8 13zm8 0c-.3 0-.6 0-1 .1 1.2.9 2 2.1 2 3.4V19h6v-2.5c0-2.3-4.7-3.5-7-3.5z', 13),
  truck: ic('M20 8h-3V4H3c-1.1 0-2 .9-2 2v11h2c0 1.7 1.3 3 3 3s3-1.3 3-3h6c0 1.7 1.3 3 3 3s3-1.3 3-3h2v-5l-3-4zM6 18.5c-.8 0-1.5-.7-1.5-1.5s.7-1.5 1.5-1.5 1.5.7 1.5 1.5-.7 1.5-1.5 1.5zm13.5-9l1.96 2.5H17V9.5h2.5zM18 18.5c-.8 0-1.5-.7-1.5-1.5s.7-1.5 1.5-1.5 1.5.7 1.5 1.5-.7 1.5-1.5 1.5z', 13),
  bank: ic('M4 10v7h3v-7H4zm6 0v7h3v-7h-3zM2 22h19v-3H2v3zm14-12v7h3v-7h-3zm-4.5-9L2 6v2h19V6l-9.5-5z', 13),
  file: ic('M14 2H6c-1.1 0-2 .9-2 2v16c0 1.1.9 2 2 2h12c1.1 0 2-.9 2-2V8l-6-6zm4 18H6V4h7v5h5v11z', 13),
  pen: ic('M3 17.25V21h3.75L17.8 9.9l-3.75-3.75L3 17.25zM20.7 7c.4-.4.4-1 0-1.4l-2.3-2.3c-.4-.4-1-.4-1.4 0l-1.8 1.8 3.75 3.75L20.7 7z', 13),
  stamp: ic('M12 3c-1.7 0-3 1.3-3 3 0 1.2.7 2.2 1.7 2.7L9.5 12H6c-1.1 0-2 .9-2 2v2h16v-2c0-1.1-.9-2-2-2h-3.5l-1.2-3.3c1-.5 1.7-1.5 1.7-2.7 0-1.7-1.3-3-3-3zM4 18v2h16v-2H4z', 22)
};

/** Shared CSS matching the Tax Invoice HTML template. */
export const getSharedPrintStyles = () => `
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; font-family: Arial, Helvetica, sans-serif; }
  ...
  .logo-section img, .logo-section svg { width: 80px; height: 80px; object-fit: contain; }
  .company-info { width: 62%; }
  .company-name { color: var(--blue-dark); font-size: 26px; font-weight: bold; margin-bottom: 6px; }
  .info-line { display: flex; align-items: flex-start; margin-bottom: 4px; gap: 8px; font-size: 11.5px; font-weight: bold; }
  .info-line-multiple { display: flex; gap: 20px; margin-bottom: 4px; }
  .gstin { font-weight: bold; color: var(--blue-dark); margin-top: 6px; font-size: 12px; }
  .copy-type {
    ...
    width: 120px; font-size: 11px; margin-top: -20px; margin-right: -20px;
  }
  .header-border { border-top: 2px solid var(--blue-dark); margin: 10px 0; }
  .title-wrapper { display: flex; align-items: center; justify-content: center; gap: 15px; margin: 14px 0; }
  .title-line { height: 2px; width: 35px; background: var(--green-main); }
  .invoice-title { color: var(--blue-dark); font-size: 20px; font-weight: bold; letter-spacing: 1px; }
  .details-grid {
    display: grid; grid-template-columns: 18% 32% 18% 32%;
    border: 1px solid var(--border-color); border-radius: 6px; overflow: hidden; margin-bottom: 14px;
  }
  .grid-item { padding: 6px 10px; border-right: 1px solid var(--border-color); border-bottom: 1px solid var(--border-color); display: flex; align-items: center; font-size: 11.5px; }
  ...
  .parties-wrapper { display: flex; gap: 12px; margin-bottom: 14px; }
  .party-box { flex: 1; border: 1px solid var(--border-color); border-radius: 6px; overflow: hidden; display: flex; flex-direction: column; }
  .party-header {
    background: var(--blue-dark); color: #fff; padding: 7px 10px; font-weight: bold;
    display: flex; align-items: center; gap: 8px; font-size: 12px; letter-spacing: 0.4px;
  }
  .party-header svg path { fill: #fff !important; }
  .party-body { padding: 10px; flex: 1; display: flex; flex-direction: column; gap: 8px; }
  .party-row { display: flex; align-items: flex-end; }
  .party-label { color: var(--blue-dark); font-weight: bold; min-width: 65px; margin-bottom: -2px; font-size: 11.5px; }
  .dotted-line {
    flex: 1; border-bottom: 1px dotted #999; min-height: 15px; font-size: 11.5px;
    padding: 0 2px 1px; font-weight: bold; color: #222;
  }
  .code-box {
    border: 1px solid var(--border-color); min-width: 45px; height: 18px; border-radius: 3px;
    text-align: center; font-weight: bold; font-size: 11px; line-height: 16px; padding: 0 4px;
  }
  .items-table { width: 100%; border-collapse: collapse; border: 1px solid var(--border-color); margin-bottom: 14px; }
  .items-table th, .items-table td { border: 1px solid var(--border-color); padding: 5px 4px; text-align: center; font-size: 11px; }
  ...
  .items-table tr.blank-row td { height: 24px; }
  .items-table tr.total-row td { font-weight: bold; }
  .total-label {
  ...
  .footer-top { display: flex; gap: 12px; margin-bottom: 14px; }
  .bank-details { flex: 6; border: 1px solid var(--border-color); border-radius: 6px; overflow: hidden; }
  .bank-body { padding: 8px 10px; }
  .bank-row { display: flex; margin-bottom: 4px; font-size: 11.5px; }
  .bank-label { color: var(--blue-dark); font-weight: bold; width: 110px; }
  ...
  .summary-table td { border: 1px solid var(--border-color); padding: 6px 10px; font-size: 11.5px; }
  ...
  .terms-body { padding: 10px 10px 10px 24px; font-size: 11px; line-height: 1.5; }
  ...
  .seal-box {
    ...
    color: var(--blue-dark); font-weight: bold; gap: 5px; min-height: 90px;
  }
  .sign-box { flex: 4; border: 1px solid var(--border-color); border-radius: 6px; overflow: hidden; display: flex; flex-direction: column; min-height: 90px; }
  .sign-area { flex: 1; display: flex; flex-direction: column; justify-content: flex-end; padding: 10px; text-align: center; }
  .sign-text { border-top: 1px solid var(--blue-dark); padding-top: 5px; color: var(--blue-dark); font-weight: bold; width: 90%; margin: 0 auto; }
  .note-block { font-size: 11px; font-weight: bold; margin-bottom: 8px; line-height: 1.45; }
`;
...
export const renderHtmlToPdf = async (html, { mode = 'save', filePrefix = 'DOC', docNo = 'N/A', width = 850 } = {}) => {
