...
export const buildTaxInvoiceHtml = (data, profileInput) => {
  ...
  const dcDate = escHtml(formatPdfDateSlash(data.dcDate) || data.dcDate || '');
  const refNo = escHtml(data.partyDocNo || data.challanNo || '');
  const refDate = escHtml(formatPdfDateSlash(data.partyDocDate) || '');
...
<div class="print-host">
  <div class="pdf-page">
    <div class="invoice-container">
      ${buildPrintCompanyHeader(profile, { showCopyBadge: true })}
      ${buildPrintTitle('TAX INVOICE')}
      ${buildDetailsGrid([
        ['Invoice No.', docNo, 'Invoice Date', docDate],
        ['Delivery Challan No.', dcNo, 'Date', dcDate],
        ['State', companyState, 'PO No./Challan No.', refNo],
        ['Code', '24', 'Date', refDate]
      ])}
