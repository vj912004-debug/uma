...
export const buildPerformaInvoiceHtml = (raw, profileInput) => {
  ...
  const pushRow = (desc, qty, rate, amt) => {
    ...
    const sgstAmt = lineAmt * (sgstCalc / 100);
    const cgstAmt = lineAmt * (cgstCalc / 100);
    const igstAmt = lineAmt * (igstCalc / 100);
    const rowTotal = lineAmt + sgstAmt + cgstAmt + igstAmt;
    totalAmt += lineAmt;
    totalSgst += sgstAmt;
    totalCgst += cgstAmt;
    totalIgst += igstAmt;
...
<style>
 ...
 .party-head svg, .box-head svg{flex-shrink:0;}
  ...
  ${FIT_FOOTER_CSS}
  .pi-page .content-wrapper tr.inv-bot { height: auto; }
  .pi-page .content-wrapper tr.inv-bot > td { height: auto; min-height: 280px; overflow: visible; }
...
<div class="content-wrapper">
  ...
  <!-- TERMS / DECLARATION / SIGNATORY -->
  ${buildFooterTerms(profile.companyName || 'UMA MICRON', formatPiPrintTermsHtml(data.terms), DEFAULT_INVOICE_DECLARATION)}

  <!-- BAR FOOTER -->
  ${buildStatusBar('Page 1 of 1', PRINT_FOOTER_MESSAGES.PI)}
...
export const renderPerformaInvoicePdf = async (data, { mode = 'save', printPrefs } = {}) => {
